...
func (this *ServerStats) launchHttpServer() {
	if config.PipedConf.StatsListenAddr == "" {
		return
	}

	this.LaunchHttpServer(config.PipedConf.StatsListenAddr, config.PipedConf.ProfListenAddr)
...
func (this *ServerStats) handleHttpQuery(w http.ResponseWriter, req *http.Request,
	...
	case "stat":
		...
		output["conf"] = *config.PipedConf

	default:
		return nil, server.ErrHttp404
	}

	if config.PipedConf.ProfListenAddr != "" {
		output["pprof"] = "http://" +
			config.PipedConf.ProfListenAddr + "/debug/pprof/"
	}
